...
- **Pre-flight quality gates** — Configurable checks block dispatch until epics meet minimum standards
- **Cross-epic dependency enforcement** — Declare dependencies between epics; dispatch is blocked until predecessors complete
- **Compliance namespace isolation** — Separate production laws from test artifacts; Barney and Judge only audit production rules
- **Scaffold hints on tasks** — Planners generate file-path hints at planning time, surfaced to implementing agents
- **Remote access via Tailscale** — Expose Dispatcher securely over your Tailscale network without opening firewall ports

## What's New (v0.3.0)

Version `0.3.0` is a major capability release covering quality gates, compliance integrity, cross-epic coordination, and AI agent reliability:

- **Pre-flight gate** — Blocks implementation dispatch when tasks lack scaffold hints, features lack acceptance criteria, or the epic description scores below threshold
- **Cross-epic dependency graph** — Epics declare predecessors; dispatch is blocked until all dependencies are completed
- **Compliance namespace** — Laws now carry a `production`/`test`/`archived` namespace; test artifacts no longer pollute audit runs
- **Scaffold hints** — Tasks carry `scaffoldHints` (file path guidance) generated by the planner and injected into the implementing agent's context
- **Planner self-scoring** — Planner agents now score their own output via MCP tools and iterate until the plan meets the quality threshold
- **Session monitor accuracy** — Task counter now reads from DB state rather than counting events; always accurate

Full release notes: [`docs/whats-new/v0.3.0.md`](./docs/whats-new/v0.3.0.md)
