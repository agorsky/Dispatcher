...
export function CasesPage() {
  ...
  const updateFilter = (key: keyof CaseFilters, value: string) => {
    setFilters((prev) => ({ ...prev, [key]: value === '_all' ? undefined : value }));
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Select value={filters.status ?? '_all'} onValueChange={(v) => updateFilter('status', v)}>
        ...
        <Select value={filters.severity ?? '_all'} onValueChange={(v) => updateFilter('severity', v)}>
        ...
        <Select value={filters.accusedAgent ?? '_all'} onValueChange={(v) => updateFilter('accusedAgent', v)}>
            ...
            <>
              ...
              <div className="flex flex-col gap-3 md:hidden">
                {cases.map((c) => (
                  <div
                    key={c.id}
                    className="rounded-lg border p-4 cursor-pointer hover:bg-accent/50 transition-colors space-y-2"
                    onClick={() => { void navigate(`/compliance/cases/${c.id}`); }}
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-mono text-sm font-semibold">Case #{c.caseNumber}</span>
                      <div className="flex gap-1.5 flex-wrap justify-end">
                        <Badge className={cn('text-xs', severityColors[c.severity] ?? '')}>{c.severity}</Badge>
                        <Badge className={cn('text-xs', statusColors[c.status] ?? '')}>{c.status}</Badge>
                      </div>
                    </div>
                    <div className="text-sm font-medium">{c.accusedAgent}</div>
                    {c.law && (
                      <div className="text-xs text-muted-foreground">
                        <span className="font-mono font-medium text-foreground">{c.law.lawCode}</span>
                        {' — '}{c.law.title}
                      </div>
                    )}
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      <span>{c.verdict ?? 'Pending verdict'}</span>
                      <span>{formatDistanceToNow(new Date(c.filedAt), { addSuffix: true })}</span>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <tbody>
                    {cases.map((c) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          {c.law ? (
                            <span className="text-xs">
                              <span className="font-mono font-medium">{c.law.lawCode}</span>{' — '}{c.law.title}
                            </span>
                        ...
                        <td className="px-4 py-3">
                          {c.verdict ? <span className="text-xs">{c.verdict}</span> : <span className="text-muted-foreground">—</span>}
                        </td>
